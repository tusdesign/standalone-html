  ...
  return (
      <Popup
       ...
       >
            ...
            <Space wrap direction='vertical' style={{ width: '100%' }}>
              {
                plates.map((_p) => (
                  <LicensePlate
                    ...
                    mode={_p.mode}
                    onConfirm={
                      (newLicensePlate: LicensePlateProps) => updateLicensePlate({ ...newLicensePlate, id: _p.id, mode: MODE.display })
                    }
          ...
          <Space direction='vertical' wrap style={{
            width: '100%', paddingLeft: '1rem', paddingRight: '1rem', boxSizing: 'border-box',
          }}>
            <Button size='large' disabled={!allowEdit} block onClick={addPlate}>添加车牌</Button>
            <Button size='large' disabled={!allowEdit || !allowSubmit} color='primary' block onClick={submitPlates}>确认</Button>
          </Space>
       </Popup>
  );
};
