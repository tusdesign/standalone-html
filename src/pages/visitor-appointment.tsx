...
const VisitorList: FC<{ value?: Visitor[] }> = ({ value }) => (
  <>
    {value && value.length ? (
      ...
      >
        {value.map((_v: Visitor) => (
          <VisitorChip name={_v.name} key={`${_v.name}-${_v.mobile}`} />
        ))}
      </Card>
    ) : (
      <Button>添加访客</Button>
...
export function VisitorAppointment() {
  ...
  const updatePlates = useCallback((newPlates: LicensePlateItem[]) => {
    ...
    form.setFieldValue(
      'plates',
      newPlates.map((_p) => `${_p.region}${_p.code}`),
    );
  }, []);
  const submit = useCallback((values: RequestData) => {
    ...
    setIsLoading(true);
    axios
      .post(`${import.meta.env.VITE_BASE_URL}/visits`, {
        visitors,
        company,
        license_plates: _p,
        name: visitee,
    ...
    <>
      ...
      >
        ...
        <Form.Item disabled={isLoading} label="来访车牌号" name="plates">
          <Button onClick={showPlateEditor}>{plates?.length ? '编辑车牌号' : '添加车牌'}</Button>
        </Form.Item>
        <Form.Item
          disabled={isLoading}
          label="访客"
          name="visitors"
          rules={[{ required: true, message: '请添加至少一个访客' }]}
          onClick={toVisitorFormPage}
        >
          <VisitorList />
        </Form.Item>
        ...
        >
          <Picker columns={[durationOptions]}>
            {(value) =>
              value && value.length && value[0]?.value ? `${value[0]?.value}小时` : '请选择'
            }
          </Picker>
        </Form.Item>
      </Form>
